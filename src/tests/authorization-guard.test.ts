...
const { resolveOrgContext, resolveProjectAccess } = vi.hoisted(() => ({
...
describe("authorization guards", () => {
  ...
  it("requires membership before granting an organization permission", async () => {
    ...
    await expect(
      assertOrgPermission("user-1", "org-1", "project.create"),
  ...
  it("does not allow members to use owner-only billing permission", async () => {
    ...
    await expect(
      assertOrgPermission("admin-1", "org-1", "billing.manage"),
    ).rejects.toMatchObject({
      status: 403,
    });
    ...
    await expect(
      assertOrgPermission("owner-1", "org-1", "billing.manage"),
    ).resolves.toMatchObject({
      organizationId: "org-1",
      role: "owner",
    });
  ...
  it("allows org admins across projects but still rejects suspended membership", async () => {
    ...
    await expect(
      assertProjectPermission("admin-1", "project-1", "task.update"),
    ).resolves.toMatchObject({
      organizationId: "org-1",
      projectId: "project-1",
    });
    ...
    await expect(
      assertProjectPermission("member-1", "project-1", "project.read"),
    ).rejects.toMatchObject({
      status: 403,
    });
  });
  ...
  it("does not let a project viewer mutate tasks", async () => {
    ...
    await expect(
      assertProjectPermission("viewer-1", "project-1", "task.update"),
    ).rejects.toMatchObject({
      status: 403,
    });

    await expect(
      assertProjectPermission("viewer-1", "project-1", "task.read"),
    ).resolves.toMatchObject({
      projectId: "project-1",
    });
  });
  ...
  it("blocks writes to archived projects for the guarded mutation permissions", async () => {
    ...
    await expect(
      assertProjectPermission("admin-1", "project-1", "project.update"),
    ).rejects.toMatchObject({
      status: 409,
      code: "conflict",
    });
    ...
    await expect(
      assertProjectPermission("admin-1", "project-1", "task.read"),
    ).resolves.toMatchObject({
      archived: true,
    });
  });
