...
/** Validate deployment-only invariants without requiring secrets during `next build`. */
export function assertProductionConfig(): void {
  ...
  if (env.storageDriver !== "cloudinary") {
    errors.push("STORAGE_DRIVER=cloudinary (Vercel has no persistent local filesystem)");
  } else {
  ...
  requireValue("SMTP_HOST", env.smtp.host);
  requireValue("MAIL_FROM", env.mailFrom);

  if (env.billingProvider === "stripe") {
    requireValue("STRIPE_SECRET_KEY", env.stripe.secretKey);
    requireValue("STRIPE_WEBHOOK_SECRET", read("STRIPE_WEBHOOK_SECRET"));
  }

  if (env.realtimeDriver === "pusher") {
    requireValue("PUSHER_APP_ID", env.pusher.appId);
    requireValue("PUSHER_KEY", env.pusher.key);
    requireValue("PUSHER_SECRET", env.pusher.secret);
    requireValue("PUSHER_CLUSTER", env.pusher.cluster);
    requireValue("NEXT_PUBLIC_REALTIME_KEY", read("NEXT_PUBLIC_REALTIME_KEY"));
    requireValue("NEXT_PUBLIC_REALTIME_CLUSTER", read("NEXT_PUBLIC_REALTIME_CLUSTER"));
  }

  if (["openai", "anthropic", "openai-compatible"].includes(env.aiProvider)) {
    requireValue("AI_API_KEY", env.aiApiKey);
    if (env.aiProvider === "openai-compatible") requireValue("AI_BASE_URL", env.aiBaseUrl);
    requireValue("AI_MODEL", env.aiModel);
  } else if (env.aiProvider !== "none") {
    errors.push(`AI_PROVIDER (unsupported value: ${env.aiProvider})`);
  }

  if (env.billingProvider !== "none" && env.billingProvider !== "stripe") {
    errors.push(`BILLING_PROVIDER (unsupported value: ${env.billingProvider})`);
  }
  if (env.realtimeDriver !== "none" && env.realtimeDriver !== "pusher") {
    errors.push(`REALTIME_DRIVER (unsupported value: ${env.realtimeDriver})`);
  }
  if (env.storageDriver !== "local" && env.storageDriver !== "cloudinary") {
    errors.push(`STORAGE_DRIVER (unsupported value: ${env.storageDriver})`);
  }
