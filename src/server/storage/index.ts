...
export async function putBytes(input: {
  ...
  if (runtime.storage.driver === "cloudinary") {
    if (!runtime.cloudinary.cloudName || !runtime.cloudinary.apiKey || !runtime.cloudinary.apiSecret) {
      throw new Error("Cloudinary storage is selected but its credentials are incomplete.");
    }
    const cloudName = runtime.cloudinary.cloudName;
    const apiKey = runtime.cloudinary.apiKey;
    const apiSecret = runtime.cloudinary.apiSecret;
...
export async function deleteObject(object: StoredObject): Promise<void> {
  try {
    if (object.provider === "cloudinary") {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      const cloudinary = require("cloudinary").v2;
      const { cloudName, apiKey, apiSecret } = (await getRuntimeConfig()).cloudinary;
