...
let pusherFingerprint = "";

async function getPusher(): Promise<typeof pusherClient> {
  const cfg = await getRuntimeConfig();
  if (cfg.realtime.driver !== "pusher") return null;
  const { appId, key, secret, cluster } = cfg.realtime;
  const fingerprint = [appId ?? "", key ?? "", secret ? "set" : "unset", cluster].join("|");
  if (!appId || !key || !secret) {
    console.error("[realtime] PUSHER_* env incomplete — disabling realtime.");
    return null;
  }
  if (pusherClient && pusherFingerprint === fingerprint) return pusherClient;
  try {
...
export async function realtimeEnabled(): Promise<boolean> {
  return (await getPusher()) !== null;
}
