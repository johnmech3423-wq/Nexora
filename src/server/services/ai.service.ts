...
import { getRuntimeConfig, type RuntimeConfig } from "@/server/services/platform-config.service";
...
function providersAvailable(config: RuntimeConfig["ai"]): { id: string; label: string; configured: boolean }[] {
  return [
    ...
    { id: "anthropic", label: "Anthropic", configured: Boolean(config.apiKey) },
  ];
}
...
function fallbackReply(feature: AiFeature, message: string, orgName: string | null, ctx?: WorkspaceContext, aiConfig?: RuntimeConfig["ai"]): string {
  if (ctx) {
    return deterministicFallback(ctx, message);
  }
  const guidance = providersAvailable(aiConfig ?? { provider: "none", apiKey: null, baseUrl: null, model: null }).filter((p) => p.id !== "none");
...
export async function chat(
  ...
  try {
    if (provider === "none") {
      ...
      text = fallbackReply(feature, message, orgName, workspaceCtx ?? undefined, aiConfig);
      provider = "none";
      fallback = true;
    } else {
      const result = await callProvider(feature, message, contextString, aiConfig);
      text = result.text;
      provider = result.provider as typeof provider;
      model = result.model;
      fallback = false;
