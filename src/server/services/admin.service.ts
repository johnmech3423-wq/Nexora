...
export async function requirePlatformAdmin(email: string): Promise<PlatformAdminContext> {
  const envAllowed = (env.platformAdminEmails ?? "")
    .split(",")
    .map((e) => e.trim().toLowerCase())
    .filter(Boolean);
  let dbAllowed: string[] = [];
  try { dbAllowed = (await getRuntimeConfig()).admins.emails; } catch { /* bootstrap check still works when DB is unavailable */ }
  if (![...envAllowed, ...dbAllowed].includes(email.toLowerCase())) {
